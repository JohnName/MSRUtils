...
import msr.msrlibrary.callback.OnTouchCallBack;
...
public class Pull2FullScreen extends LinearLayout implements View.OnTouchListener {
    ...
    public Pull2FullScreen(Context context, AttributeSet attrs) {
        super(context, attrs);
        setOnTouchListener(this);
    }

    public Pull2FullScreen(Context context) {
        super(context);
    }
    ...
    private void half2None() {
        float[] values = new float[]{move_height, getHeight() - BUTTONSiZE};
        startAnimation(values);
        //接口得到当前视图显示类型
        ontouchCallBack.slideState(NONE);
        setType(NONE);
    }
    ...
    private void all2Half() {
        ...
        startAnimation(values);
        //接口得到当前视图显示类型
        ontouchCallBack.slideState(HALF);
        setType(HALF);
    }
    ...
    private void half2All() {
        ...
        //接口得到当前视图显示类型
        ontouchCallBack.slideState(ALL);
        setType(ALL);
    }
    ...
    private void all2None() {
        float[] values = new float[]{0, getHeight() - BUTTONSiZE};
        startAnimation(values);
        ontouchCallBack.slideState(NONE);
        setType(NONE);
    }
    ...
    public void none2Half() {
        float[] values = new float[]{getHeight() - BUTTONSiZE, move_height};
        startAnimation(values);
        ontouchCallBack.slideState(HALF);
        setType(HALF);
    }
    ...
    public void none2All() {
        float[] values = new float[]{getHeight() - BUTTONSiZE, 0};
        startAnimation(values);
        ontouchCallBack.slideState(HALF);
        setType(HALF);
    }
     ...
     * @param init
     */
    public void initDisplay(boolean init) {
        if (init) {
            all2Half();
        }
    }

    public void startSlide(OnTouchCallBack ontouchCallBack) {
